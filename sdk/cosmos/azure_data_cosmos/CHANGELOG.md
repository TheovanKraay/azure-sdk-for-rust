...
- Added the `ReadConsistencyStrategy::LatestCommitted` variant, which reads the latest committed version from the region in preferred order. Any explicitly requested non-`Default` read consistency strategy is now serialized on the wire via the `x-ms-cosmos-read-consistency-strategy` header so the Gateway applies the requested read consistency independent of the account's default consistency level.
